...
amz <- dija[dija$Name=="AMZN",]
amz$Date <- as.Date(amz$Date)
amz_lag <- embed(amz$Open, 6)
amz_y <- amz$Open[7:length(amz$Open)]
...
ar1 <- runAR_k(amz_y, amz_lag, 0.1, q = 0, nu = 2, k = 1)
ar2 <- runAR_k(amz_y, amz_lag, 0.1, q = 0, nu = 2, k = 2)
ar3 <- runAR_k(amz_y, amz_lag, 0.1, q = 0, nu = 2, k = 3)
ar4 <- runAR_k(amz_y, amz_lag, 0.1, q = 0, nu = 2, k = 4)
ar5 <- runAR_k(amz_y, amz_lag, 0.1, q = 0, nu = 2, k = 5)
ar6 <- runAR_k(amz_y, amz_lag, 0.1, q = 0, nu = 2, k = 6)
...
for(i in 1:N){
  ...
  pi.m[i,] <- maj.int
  pi.wm[i,] <- wmaj.int
  m.cov[i] <- I(maj.int[1] <= amz_y[i+t] && amz_y[i+t] <= maj.int[2])
  wm.cov[i] <- I(wmaj.int[1] <= amz_y[i+t] && amz_y[i+t] <= wmaj.int[2])
  if(i %% 100 == 0){
