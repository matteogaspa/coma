...
f_eta_loss <- ggplot(data.plot, aes(x = iter)) +
  ...
  geom_line(aes(y = ar, color = "AR(2,0)"), linetype = "solid", size = 1) +
  geom_line(aes(y = armax, color = "ARMAX(2,0)"), linetype = "solid", size = 1) +
  geom_line(aes(y = aci, color = "COMA"), linetype = "dashed", size = 1) +
  labs(title = expression(eta==0.1), x = "Iter", y = "Loss", color = "") +
  scale_color_manual(values = c("LM" = "red", "AR(2,0)" = "forestgreen", "ARMAX(2,0)" = "blue", "COMA" = "orange", "Rand. Weighted Maj." = "orange")) +
...
grid.arrange(a_eta_err, a_eta_wts, f_eta_err, f_eta_wts, ncol= 4)
