...
pM1
pM1<-ggplot(data.plot, aes(x = iter)) +
  geom_line(aes(y = local.m), color = "forestgreen", linetype = "solid") +
  geom_line(aes(y = local.wm), color = "orange", linetype = "dashed") +
  geom_hline(yintercept = 0.80, color = "black", linetype = "dashed") +
  labs(title = expression("Adaptive"~eta), x = "Iter", y = "Local Level Coverage", color = "") +
  scale_color_manual(values = c("Weighted Majority" = "forestgreen", "Rand. Weighted Majority" = "orange", "Bern 0.9" = "orange")) +
  theme_minimal() + theme(legend.position = "bottom") + ylim(0.75, 1)
...
plot_tot   <- grid.arrange(plot_models, plot_merge, ncol = 2)
grid.arrange(p_lm1, p_ar2, p_armax1, pM1, pM2, ncol = 5)
